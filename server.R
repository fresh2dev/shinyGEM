...
stringsAsFactors <- function (dt) {
  ...
  dt <- dt[, lapply(.SD, function(x) {
  	if (is.logical(x)) {
  		...
  		new_x <- if (!is.factor(x)) { x } else { as.character(x) }
	      ...
	      as.numeric(sub(new_x, pattern='%', replacement = '', fixed=T)) / 100
	    } else {
	      factor(gsub(new_x, pattern="'", replacement = "`", fixed=T))
	    }
		}
  })]
...
function(input, output, session) {
  ...
  observe({
    ...
    if (input$presetDataset == 'Game Ratings') {
      ...
      dataset <- readRDS('college.RDS') %>% stringsAsFactors()
  ...
  n$x_breaks <- reactive({
  	n <- 30L
  	if (!is.na(input$n_x_breaks) && as.integer(input$n_x_breaks) > 0) {
  		n <- as.integer(input$n_x_breaks)
  	}
  	return(n)
  })
  ...
  n$y_breaks <- reactive({
  	n <- 10L
  	if (!is.na(input$n_y_breaks) && as.integer(input$n_y_breaks) > 0) {
  		n <- as.integer(input$n_y_breaks)
  	}
  	return(n)
  })
  ...
  facet_features <- function(p, x_lab=NULL, y_lab=NULL, no_legend = FALSE,
  	...
  	if (nchar(input$xlab) > 0) {
  		if (grepl(input$xlab, pattern='^\\s+$')) {
  			x_lab <- NULL
  		} else {
  			x_lab <- input$xlab
  		}
  	...
  	if (nchar(input$ylab) > 0) {
  		if (grepl(input$ylab, pattern='^\\s+$')) {
  			y_lab <- NULL
  		} else {
  			y_lab <- input$ylab
  		}
  ...
  plot$height <- reactive({
  	if (!between(input$plotHeight, 3, 120)) { 10L } else { as.integer(input$plotHeight) }
  })

  plot$width <- reactive({
  	if (!between(input$plotWidth, 3, 120)) { 16L } else { as.integer(input$plotWidth) }
  })
